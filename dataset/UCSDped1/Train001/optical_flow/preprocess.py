...
def computeOpticalFlow(src_location,dest_location_opt,dest_location_img):
	print (src_location)
	...
	while(1):
		...
		if k == 27:
		    break
		cv.imwrite(dest_location_img + frames[num]+ '.png',next)
		cv.imwrite( dest_location_opt + frames[num]+ '.png',bgr)
		prvs = next
...
def baba(src_location,dest_location_opt,dest_location_img):
	...
	for i in mainDir:
		os.chdir(i)
		computeOpticalFlow(os.getcwd(),dest_location_opt,dest_location_img)
